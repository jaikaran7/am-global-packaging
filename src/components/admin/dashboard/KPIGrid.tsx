...
function buildKpis(data: DashboardData | null | undefined): KPIItem[] {
  const rev = data?.totalRevenueThisMonth ?? 0
  const revPct = data?.revenueChangePct ?? 0
  let revDelta: string | undefined
  if (revPct !== 0) {
    const sign = revPct > 0 ? '+' : ''
    revDelta = `${sign}${revPct.toFixed(1)}% from last month`
  } else {
    revDelta = undefined
  }
  ...
  return [
    ...
    },
    {
      ...
      delta: lowStock > 0 ? `${lowStock} low stock` : undefined,
      deltaPositive: false,
...
export default function KPIGrid({ data }: Readonly<KPIGridProps>) {
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      {kpis.map((kpi, idx) => (
        <motion.div
          ...
          className="glass glass--soft p-4 rounded-xl admin-card-warm cursor-pointer"
        >
          <div className="flex items-start justify-between">
            <div className="flex-1 min-w-0">
              <p className="text-[11px] font-semibold text-[#6b7280] uppercase tracking-wider mb-1.5">{kpi.title}</p>
              <AnimatedCount
                value={kpi.value}
                prefix={kpi.title === 'Total Revenue' ? '$' : ''}
                className="text-xl font-extrabold text-[#2b2f33]"
                duration={1.6}
              />
              {kpi.delta && (
            ...
            <div className={`w-9 h-9 rounded-lg ${kpi.iconBg} flex items-center justify-center flex-shrink-0 shadow-[0_2px_6px_rgba(0,0,0,0.04)] border border-white/50`}>
              {kpi.icon}
            </div>
          </div>
        </motion.div>
