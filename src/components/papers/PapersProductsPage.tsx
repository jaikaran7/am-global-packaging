      ...
      <div className={`p-4 md:p-5 flex flex-col ${viewMode === "list" ? "flex-1 min-w-0" : ""}`}>
        ...
        {priceRange && (
          <p className="text-xs font-semibold text-forest mt-2">
            {priceRange}
            <span className="text-warm-gray font-normal"> excl. GST · +10% GST at checkout</span>
          </p>
        )}

        {/* Feature badges (desktop) */}
        {product.feature_badges.length > 0 && (
          <div className="hidden md:flex flex-wrap gap-1.5 mt-3">
            {product.feature_badges.slice(0, 3).map((badge) => (
              <span
                key={badge}
                className="px-2 py-0.5 bg-forest/6 text-forest text-[10px] font-semibold rounded-full"
              >
                {badge}
              </span>
            ))}
        ...
        {/* CTAs */}
        <div className="mt-4 pt-4 border-t border-kraft/8 flex items-center justify-between gap-2">
          <Link
            href={`/papers/product/${product.slug}`}
            className="inline-flex items-center gap-1 text-xs font-semibold text-forest hover:text-kraft transition-colors"
          >
        ...
        <div className="grid lg:grid-cols-[260px_1fr] gap-10 lg:gap-14">
          ...
          <aside className="hidden lg:block">
            <div className="sticky top-28 space-y-8">
              ...
              {/* Eco badges panel */}
              <div className="bg-white rounded-2xl p-5 border border-kraft/8">
                <h4 className="font-bold text-sm text-charcoal mb-3">Every Sheet Is</h4>
                <div className="space-y-2">
                  {["100% Handmade", "Acid Free", "Tree Free", "99% Recycled", "Ships Globally"].map((b) => (
                    <div key={b} className="flex items-center gap-2 text-xs text-charcoal/70">
